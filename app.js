const C=window.PENGU_CONFIG;
const state={candles:{},price:null,ticker:null,trades:[],connected:false,lastPacket:0};
const $=id=>document.getElementById(id);
const fmt=(n,d=8)=>Number.isFinite(n)?n.toFixed(d):"—";
const pct=n=>Number.isFinite(n)?`${n>=0?"+":""}${n.toFixed(2)}%`:"—";
function priceDigits(p){if(!Number.isFinite(p))return 8;if(p>=1000)return 2;if(p>=1)return 4;if(p>=.1)return 5;return 8}
...
function rsi(a,n=14){if(a.length<n+1)return 50;let g=0,l=0;for(let i=1;i<=n;i++){const d=a[i]-a[i-1];g+=Math.max(d,0);l+=Math.max(-d,0)}let ag=g/n,al=l/n;for(let i=n+1;i<a.length;i++){const d=a[i]-a[i-1];ag=(ag*(n-1)+Math.max(d,0))/n;al=(al*(n-1)+Math.max(-d,0))/n}return al===0?100:100-(100/(1+ag/al))}
function stats(tf){const a=state.candles[tf]||[],c=a.map(x=>x.c),v=a.map(x=>x.q),p=c.at(-1),e9=ema(c,9),e21=ema(c,21),avg=v.slice(-20).reduce((x,y)=>x+y,0)/(Math.min(20,v.length)||1);return{p,e9,e21,rsi:rsi(c),volRatio:avg?v.at(-1)/avg:1,ret3:c.length>3?(p/c.at(-4)-1)*100:0,ret10:c.length>10?(p/c.at(-11)-1)*100:0}}
function decision(){const a=stats("1m"),b=stats("5m"),c=stats("15m");let score=0,reasons=[];
if(b.p>b.e9&&b.e9>b.e21){score+=2;reasons.push(["5m trend","Bullish","good"])}else if(b.p<b.e9&&b.e9<b.e21){score-=2;reasons.push(["5m trend","Bearish","bad"])}else reasons.push(["5m trend","Mixed","warn"]);
if(c.p>c.e21){score++;reasons.push(["15m trend","Above EMA21","good"])}else{score--;reasons.push(["15m trend","Below EMA21","bad"])}
if(b.rsi>=72){score--;reasons.push(["5m RSI",b.rsi.toFixed(1)+" overbought","warn"])}else if(b.rsi<=35){score++;reasons.push(["5m RSI",b.rsi.toFixed(1)+" weak/oversold","good"])}else reasons.push(["5m RSI",b.rsi.toFixed(1)+" neutral","neutral"]);
...
function connect(){
  async function poll(){
    try{
      ...
      setText("latency",`${Date.now()-started}ms`);
